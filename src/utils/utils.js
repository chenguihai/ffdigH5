...
export default {
    ...
    insertLoginRecordHttp(state, userId = '', msg = '') {  //插入登录记录信息
        ...
        axiosHttp(`api/Record/UserRecord/InsertLoginRecord?userId=${userId}&state=${state}&type=M&errorMsg=${msg}`, '', 'get').then((res) => {
        }).catch(e => {
            console.log(e);
        });
    },
    ...
    openWeixinCommonFun(url) {  //微信公众跳转链接
        let ua = navigator.userAgent.toLowerCase();
        let isWeixinLogin = window.sessionStorage.getItem('isWeixinLogin');
        if (ua.indexOf('micromessenger') !== -1 && isWeixinLogin !== 'isLogin') {
            window.sessionStorage.setItem('isWeixinLogin', 'isLogin');
            window.location.replace(`https://open.weixin.qq.com/connect/oauth2/authorize?appid=${config.appID}&redirect_uri=${config.serverUrl}${url}&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect`);
        }
    },
            ...
            if (nickNameId) {
                ...
                nickNameId.addEventListener('compositionend', () => {
                    ...
                    if (this.byteLen(value) > maxlength) {
                        ...
                        nickNameId.setAttribute('value',this.getMaxlen(value, maxlength));
                    }
                });
